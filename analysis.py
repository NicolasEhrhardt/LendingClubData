...
def import_data(file_names=FILENAMES):
    data_chunks = [
        pd.read_csv(file_name, skiprows=1, na_values='n/a')
...
def get_data(file_names=FILENAMES):
    data_raw = import_data(file_names)
...
def plot_tree(clf, X, filename='tree.dot'):
